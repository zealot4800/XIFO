...
if __name__ == '__main__':
    ...
    for x in lambdas:
        ...
        file = "temp/sppifo/sppifo_evaluation/fairness/web_search_workload/"+str(x)+"/XIFOWFQ_8/analysis/flow_completion.statistics"
        r = open(file, 'r')
        lines = r.readlines()
        for i, line in enumerate(lines):
            if "less_100KB_mean_fct_ms" in line:
    ...
    w.write("#    TCP    DCTCP    AFQ_8    SPPIFOWFQ_8    PIFOWFQ_8   XIFOWFQ_8\n")
    ...
    for x in lambdas:
        ...
        file = "temp/sppifo/sppifo_evaluation/fairness/web_search_workload/"+str(x)+"/XIFOWFQ_32/analysis/flow_completion.statistics"
        r = open(file, 'r')
        lines = r.readlines()
        for i, line in enumerate(lines):
            if "less_100KB_mean_fct_ms" in line:
    ...
    w.write("#    TCP    DCTCP    AFQ_32    SPPIFOWFQ_32    PIFOWFQ_32    XIFOWFQ_32\n")
    w.write("4000   %s    %s    %s    %s    %s    %s\n" % (FCTs[0][0], FCTs[0][1], FCTs[0][2], FCTs[0][3], FCTs[0][4], FCTs[0][5]))
    w.write("6000   %s    %s    %s    %s    %s    %s\n" % (FCTs[1][0], FCTs[1][1], FCTs[1][2], FCTs[1][3], FCTs[1][4], FCTs[1][5]))
    w.write("10000   %s    %s    %s    %s    %s    %s\n" % (FCTs[2][0], FCTs[2][1], FCTs[2][2], FCTs[2][3], FCTs[2][4], FCTs[2][5]))
    w.write("15000   %s    %s    %s    %s    %s    %s\n" % (FCTs[3][0], FCTs[3][1], FCTs[3][2], FCTs[3][3], FCTs[3][4], FCTs[3][5]))
    ...
    r = open("temp/sppifo/sppifo_evaluation/fairness/web_search_workload/14150/XIFOWFQ_32/analysis/flow_completion.statistics", 'r')
    lines = r.readlines()
    for i, line in enumerate(lines):
        if "mean_fct_ms" in line:
            if line.split("=")[0] == "10KB_mean_fct_ms":
